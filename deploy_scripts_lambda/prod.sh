...
IMAGE_NAME=chatnote_prod
ECR_PATH=974539925060.dkr.ecr.ap-northeast-2.amazonaws.com/$IMAGE_NAME

docker build -t $IMAGE_NAME:$TAG . -f dockerfiles/prod.lambda.Dockerfile
...
DELETING_IMAGES_2=$(docker images --filter=reference='chatnote_prod*:*' -q)
